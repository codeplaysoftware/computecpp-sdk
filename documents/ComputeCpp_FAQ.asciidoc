...
built with any C++ compiler to produce the native pass:[C++] executable and the
ComputeCpp device compiler is only used for the SYCL device kernels.
More information can be found at
<<ComputeCpp_Integration_Guide.asciidoc#, ComputeCpp Integration Guide>>
...
function can execute on the
<<ComputeCpp_Glossary.asciidoc#SYCLHostDevice, SYCL Host Device>>
allowing developers to use their prefered C++ debuggers to debug their code on
the host CPU. ComputeCpp(TM) Community Edition does not provide a SYCL device
